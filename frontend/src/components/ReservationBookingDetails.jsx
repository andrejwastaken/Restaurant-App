...
function ReservationBookingDetails({
    ...
    const {
        selectedInitialDate,
        selectedDay, // This is the UTC day (0=Sun, 1=Mon...), passed from calendar
        selectedTimeSlot,
    ...
    // Create a Date object by parsing the string as UTC for display or UTC calculations.
    ...
    const durationOptions = useMemo(() => {
        if (!selectedDateUTC || !selectedTimeSlot || !restaurantOperatingHours || !restaurantSpecialDays) {
            return [];
        }

        let closeTimeStr = null;

        // Directly use the 'selectedInitialDate' string. No more error-prone conversions.
        const specialDay = restaurantSpecialDays.find(d => d.day === selectedInitialDate);

        if (specialDay) {
            closeTimeStr = specialDay.close_time;
        } else {
            // Use getUTCDay() for the correct day of the week.
            const dayOfWeek = selectedDateUTC.getUTCDay(); // 0=Sun, 6=Sat
            const mondayBasedDay = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // 0=Mon, ... 6=Sun
        ...
        const options = [];
        for (let d = restaurantTimeSlot; d <= maxDuration; d += restaurantTimeSlot) {
            options.push(d);
        }
        return options;
    }, [selectedInitialDate, selectedTimeSlot, restaurantOperatingHours, restaurantSpecialDays, restaurantTimeSlot, selectedDateUTC]);
    ...
    const handleFindTables = async () => {
        const params = {
            ...
            day: selectedDay, // Use the pre-calculated UTC day
            // Send the clean 'YYYY-MM-DD' string directly to the backend.
        ...
        try {
            ...
            const errorMessage = error?.response?.data?.error || "An error occurred while fetching tables.";
    ...
    const displayDate = selectedDateUTC ? selectedDateUTC.toLocaleDateString("en-US", {
        weekday: "long",
        month: "long",
        day: "numeric",
        timeZone: "UTC", // Ensure display formatting uses UTC
    }) : "";
            ...
            <div className="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
                ...
                <div className="space-y-6">
                    ...
                    >
                        ...
                        >
                            <option value="" disabled>
                                {durationOptions.length > 0 ? "Select duration..." : "No durations available"}
                            </option>
