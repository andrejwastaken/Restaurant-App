...
const CanvasTable = ({ table, isUnavailable, isSelected, onSelect }) => {
	const baseClasses =
		"absolute rounded-md w-16 h-16 flex items-center justify-center font-semibold text-white shadow-lg transform transition-all duration-200";
...
function AvailableTablesPage() {
	...
	const { availableTables, unavailableTables, reservationDetails } =
		location.state;
	...
	const handleSelectTable = (tableId) => {
		setSelectedTableId((prevSelectedId) =>
			prevSelectedId === tableId ? null : tableId
		);
	};
				...
				<main className="flex-grow bg-white border border-gray-200 rounded-lg shadow-inner overflow-hidden relative p-4">
					<div className="w-full h-full relative">
						...
						{unavailableTables.map((table) => (
							<CanvasTable key={table.id} table={table} isUnavailable={true} />
						))}
					</div>
