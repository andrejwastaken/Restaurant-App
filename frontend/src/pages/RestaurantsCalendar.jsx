...
function RestaurantsCalendar() {
    ...
    const isOperatingDay = useCallback((date) => {
    ...
    const timeSlots = useMemo(generateTimeSlots, [selectedDate, isOperatingDay, operatingHoursMap, restaurantTimeSlot]);
                        ...
                        {selectedDate && (
                            <div className="p-4 bg-amber-50 rounded-md">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-amber-900 font-semibold">Your Selection</p>
                                        <p className="text-amber-800 text-sm">
                                            {selectedDate.toLocaleDateString('en-GB')}
                                            {selectedTimeSlot ? ` at ${selectedTimeSlot}` : ' - Select a time'}
                                        </p>
                                    </div>
                                    <button
                                        disabled={!selectedTimeSlot} 
                                        className="px-4 py-2 font-bold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed transform hover:scale-105"
                                    >
                                        Add Info
                                    </button>
                                </div>
